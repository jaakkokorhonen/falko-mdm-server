...
def poll_loop() -> None:
    ...
    while True:
        if not CONFIG.token_path.exists():
            logger.error("Token file missing. Sleeping before retry.")
            time.sleep(60)
            continue
        ...
        url = f"{CONFIG.server_url}/linux/mdm/{inventory.collect()['device_id']}"
        try:
            ...
            if res.status_code == 200:
                ...
                if latest_agent_version and latest_agent_version != AGENT_VERSION:
                    logger.warning(
                        "New agent version available: %s (installed: %s). Auto-update is out of scope in MVP.",
                        latest_agent_version,
                        AGENT_VERSION,
                    )
                ...
                logger.error(
                    "Authentication failure (401) on poll. Backing off."
                )
                time.sleep(300)
                continue
