...
Korjaukset (2026-07):
  ...
  - SEC-11: require_admin-dekoraattori — roolitarkistus /users-endpointille.
  - list_devices tukee sivutusta, command_type validoitu allowlistillä.
...
def _get_authenticated_email() -> str | None:
    """Palauttaa autentikoituneen käyttäjän sähköpostin tai None.
...
def require_auth(f: Callable[..., Any]) -> Callable[..., Any]:
    ...
    Käyttäjältä vaaditaan autentikoitu identiteetti (IAP tai Bearer) sekä
    Firestoren authorized-status (tai bootstrap-admin).
    """
    @wraps(f)
...
def list_all_devices() -> Response:
    ...
    Query-parametrit:
      ...
      cursor (str):           Edellisen sivun viimeinen UDID.
    """
    try:
...
def list_all_users() -> Response:
    ...
    Vain admin-roolin käyttäjät voivat kutsua tätä endpointtia (SEC-11).
